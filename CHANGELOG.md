...
- `devos test` command: Tester suite via `commands.test` (fallback `bun test`).
- `devos qa` command: QA suite via `commands.qa` (fallback `npm run lint`); tracked separately from Tester.
