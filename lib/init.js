...
async function confirmSetup(detection, interactive, overrides) {
  ...
  console.log(`  Test     : ${detection.commands.test}`);
  ...
  const patchAns = await question(
    rl,
    `\n${colors.cyan}Opt-in: merge missing package.json lint/test scripts if absent? [y/N]: ${colors.reset}`
  );
...
async function runInit(options) {
  ...
  if (setup.applyPatches) {
    ...
    if (fs.existsSync(pkgPath)) {
      const toAdd = {};
      // Only add a test script when we have a safe, runtime-native default; never invent lint tooling.
