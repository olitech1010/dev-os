...
This document defines the full multi-agent team. Each agent has a single role, defined constraints, and a system prompt in `agents/prompts/`.
...
Checks:
- Code matches `CODING_STANDARDS.md` conventions
...
- Validates that the deployment checklist in `docs/guides/deployment-checklist.md` is complete
...
2. Architect → applies `grill-me.md` skill in a single zero-shot execution
