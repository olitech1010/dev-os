...
![npm](https://img.shields.io/badge/npm-dev--os-red.svg)
...
npm install -g dev-os
devos init
```

> [!NOTE]
> The npm package is named **`dev-os`** (the name `devos` on npm belongs to an unrelated 2016 package). Once installed, the command you type is `devos`.
...
```bash
npx dev-os init --existing --stack nextjs
```
...
├── LICENSE                     # MIT
├── VERSION
├── package.json                # npm package: dev-os (bin: devos)
...
npm publish            # publishes dev-os to the registry
```
