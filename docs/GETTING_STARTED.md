...
> **Note on the package name:** The npm registry package is **`dev-os`**. The name `devos` on npm belongs to an unrelated package — do not install it. Once installed, the CLI binaries are `devos`, `olives-devos`, and `devos-init`.
...
npx dev-os init
```
*(Or install globally: `npm install -g dev-os`. After a global install, the command is simply `devos` — the bin name — so you run `devos init`. With `npx` you use the package name `dev-os`.)*

### Option B: Non-Interactive Flag Execution
For automated environments, CI scripts, or quick setup:
```bash
npx dev-os init --stack nextjs --existing
```
